    ...
    partial class Travian
    {
        ...
        private void NewParseResource(int VillageID, string data)
        {
            if (VillageID == 0)
                return;
            MatchCollection m;
            m = Regex.Matches(data, RegexLang["newparser_ParseResource"], RegexOptions.Singleline);
            if (m.Count == 4)
                for (int i = 0; i < 4; i++)
        ...
        private int NewParseTribe()
        {
            ...
            if (data == null)
                return 0;
            Match m = Regex.Match(data, RegexLang["newparser_ParseTribe"], RegexOptions.Singleline);
